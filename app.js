...
app.webhooks.on('installation_repositories.added', async ({ octokit, payload }) => {
  console.log(`Received a installation_repositories event for #${payload.installation.id}`)
})

app.webhooks.on('installation.deleted', async ({ octokit, payload }) => {
  console.log(`Received a installation.deleted event for #${payload.installation.id}`)
})

app.webhooks.on('installation_repositories.removed', async ({ octokit, payload }) => {
  console.log(`Received a installation_repositories event for #${payload.installation.id}`)
})
